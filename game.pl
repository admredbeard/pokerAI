...
go(Deck, Loop) :-
  ...
  whoWon(P1seven,P2seven, Winner, V1, V2),
  ...
  (Winner == p1 -> add(V1, win, P1Name), add(V2, loss, P2Name) %only for preflop atm "preflop" can be replaced by anything
  ;Winner == p2 -> add(V1, loss, P1Name), add(V2, win, P2Name)
  ;Winner == tie -> !
  ),
  Y is Loop - 1,
  go(Shuffled, Y).
